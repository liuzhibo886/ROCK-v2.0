...
public class UtilCacheKey {
	...
	public static String getParam(String[] names, Object[] params, String parameter) {
		...
		if (StringUtils.isBlank(parameter)) {
			throw new RockException(ResultEnum.AOP_ERR, "parameter 为空");
		}
		if (parameter.endsWith(".")) {
			throw new RockException(ResultEnum.AOP_ERR, "parameter不能以(.) 结尾");
		}
		...
		if (parameter.indexOf(".") > -1) {
			...
			if (params.length <= index) {
				throw new RockException(ResultEnum.AOP_ERR, "缓存key错误，检查是否跟属性保持一致");
			}
			// 取值
