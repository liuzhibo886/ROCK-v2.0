...
public class UtilString extends StringUtils {
	 ...
	 * 验证码数组长度
	 */
	private static Integer NUMBER_CHAR_LENGTH = NUMBER_CHAR.length;
	/**
	 * 字符串数组
	 */
	private static char[] STRING_CHAR = "abcdefghijklmnopqrstuvwxyz0123456789".toCharArray();
	/**
	 * 字符串数组长度
	 */
	private static Integer STRING_CHAR_LENGTH = STRING_CHAR.length;
	/**
	 ...
	 * abbr2 使用，取出标记
	 */
	private static final Pattern ABBR2_PATT = Pattern.compile("<([a-zA-Z]+)[^<>]*>");

	/**
	 * 判断是否为整数
	 */
	private static final Pattern IS_INTEGER_PATT = Pattern.compile("<([a-zA-Z]+)[^<>]*>");
	...
	public static String getRandomNumber(int size) {
		...
		for (int i = 0; i < size; i++) {
			// 取随机字符索引
			int n = ran.nextInt(NUMBER_CHAR_LENGTH);
			// 记录字符
			sb.append(NUMBER_CHAR[n]);
		}
		return sb.toString();
	}
	...
	public static String getRandomString(int length) {
		...
		for (int i = 0; i < length; i++) {
			int number = random.nextInt(STRING_CHAR_LENGTH);
			sb.append(STRING_CHAR[number]);
		}
		return sb.toString();
	}
	...
	public static String abbr2(String param, int length) {
		...
		// 用正则表达式取出标记
		Matcher m = ABBR2_PATT.matcher(tempResult);
	 ...
	 * @return 是否为空或等于0
	 */
	public static boolean isIntEmptyOrZero(Integer param) {
	...
	public static boolean isInteger(String str) {
		return IS_INTEGER_PATT.matcher(str).matches();
	}
	...
	public static String hideTelephone(String telephone) {
		if (UtilString.isNotBlank(telephone)) {
			if (telephone.length() > 8) {
				String telephoneStart = null;
				String telephoneEnd = null;
				// 手机号
				if (telephone.startsWith("0")) {
					telephoneStart = telephone.substring(0, 4);
					telephoneEnd = telephone.substring(8);
				} else {
					telephoneStart = telephone.substring(0, 3);
					telephoneEnd = telephone.substring(7);
				}
				// 电话号码
				return telephoneStart + "****" + telephoneEnd;
			} else {
				return telephone;
			}
		}
	...
	public static boolean hasBlank(String... strs) {
		if (UtilCollection.isEmpty(strs)) {
			return true;
		}
		for (String str : strs) {
			if (isBlank(str)) {
...
//	}
	...
	public static String format(String template, Object... values) {
		if (UtilCollection.isEmpty(values) || isBlank(template)) {
			return template;
		}
	...
	public static ByteBuffer byteBuffer(String str, String charset) {
		return ByteBuffer.wrap(UtilString.bytes(str, charset));
	}
